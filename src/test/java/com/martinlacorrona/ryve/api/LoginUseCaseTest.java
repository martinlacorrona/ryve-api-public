...
class LoginUseCaseTest {
    ...
    @Test
    void loginWithNoRegisteredUser() throws Exception {
        String mailNotRegistered = "njdsandsjkandsjka@ndsjakndsjka";
        ...
        mockMvc.perform( MockMvcRequestBuilders
                ...
                .accept(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION,
                        "Basic " + Base64Utils.encodeToString((mailNotRegistered + ":" + password).getBytes())))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void loginBadPassword() throws Exception {
        String passwordBad = "aaaaaaaaa";
        mockMvc.perform( MockMvcRequestBuilders
                .get("/v1/user")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION,
                        "Basic " + Base64Utils.encodeToString((mail + ":" + passwordBad).getBytes())))
                .andExpect(status().isUnauthorized());
    }
